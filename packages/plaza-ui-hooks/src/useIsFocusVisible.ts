...
function isFocusVisible(event: React.FocusEvent<HTMLInputElement>) {
  const {target} = event;
  try {
    return (target as HTMLInputElement).matches(':focus-visible');
  } catch (error) {
...
export function useIsFocusVisible() {
   ...
   * Should be called if a blur event is fired
   */
  function handleFocusVisible(event: React.FocusEvent<HTMLInputElement>) {

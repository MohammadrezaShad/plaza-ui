...
export const usePopover = ({
  ...
  const scoutRef = useElementRef('plaza-ui-popover-scout', {
    position: 'fixed',
    top: '0px',
    left: '0px',
    width: '0px',
  ...
  const positionPopover = useCallback<PositionPopover>(
    ({
      ...
      if (contentLocation) {
        ...
        popoverRef.current.style.transform = `translate(${
          left - scoutRect.left
        }px, ${top - scoutRect.top}px)`;
      ...
      popoverRef.current.style.transform = `translate(${
        finalLeft - scoutRect.left
      }px, ${finalTop - scoutRect.top}px)`;
